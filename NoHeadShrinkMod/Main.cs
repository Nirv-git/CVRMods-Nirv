...
namespace NoHeadShrinkMod
{
    ...
    internal class HarmonyPatches
    {
        ...
        [HarmonyPrefix]
        [HarmonyPatch(typeof(ABI_RC.Core.Player.AvatarClone), nameof(AvatarClone.MyOnPreCull))]
        public static bool OnMyOnPreCull(AvatarClone __instance, Camera cam)
        {
            if (Main.disableHeadShrink.Value)
            {
                if (!__instance.isActiveAndEnabled)
                return false;
                if ((Main.disableHeadShrink.Value ? 0 : (AvatarClone.IsHeadHiddenCamera(cam) ? 1 : 0)) != 0)
                {
                    __instance.ConfigureSourceShadowCasting(true);
                    __instance.ConfigureCloneVisibility(true, false);
                }
                else if ((Main.disableHeadShrink.Value ? 0 : (AvatarClone.IsUiCullingCamera(cam) ? 1 : 0)) != 0)
                {
                    __instance.ConfigureCloneVisibility(true, true);
                }
                else
                {
                    __instance.ConfigureSourceShadowCasting(false);
                    __instance.ConfigureCloneVisibility(false, false);
                }
